...
def choose_action(state, mdp_data):
    ...
    value = mdp_data["value"].reshape(-1,1)
    #reward0 = np.sum(transition_prob[state,:, 0] * value)
    #reward1 = np.sum(transition_prob[state,:, 1] * value)
    #if (reward0 > reward1):
        #action = 0
    ...
    action = np.sum(transition_prob[state] * value, axis=0)
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    num_state = np.shape(transition_counts)[0] # 163
    # update transition probability
    for a in range(2):
        for s in range(num_state):
            total = np.sum(transition_counts[s,:,a]) # # times take action a in each state
            if total > 0: # state-action pair that has been tried
                transition_prob[s,:,a]  = transition_counts[s,:,a] / total
    # update reward
    for s in range(num_state):
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    # *** START CODE HERE ***
    transition_prob = mdp_data["transition_probs"]
    num_state = np.shape(mdp_data["transition_counts"])[0]
    iter = 0
    max_action = np.zeros((num_state,1))
    while True:
        iter += 1
        value0 = np.dot(transition_prob[:, :, 0] , mdp_data["value"])
        value1 = np.dot(transition_prob[:, :, 1] , mdp_data["value"])
        max_action = np.maximum(value0, value1) # element-wise maximum (163,)
        new_value = mdp_data["reward"] + gamma * max_action
        diff = np.max(np.abs(mdp_data["value"]-new_value))
