...
def forward_prop(data, labels, params):
    ...
    """
    # *** START CODE HERE ***
    W1 = params['W1']  # (784, 300)
    W2 = params['W2']  # (300, 10)
    b1 = params['b1']  # (1, 300)
    b2 = params['b2']  # (1, 10)

    z1 = data @ W1 + b1  # (1000, 300)
    a = sigmoid(z1)  # (1000, 300)
    z2 = a @ W2 + b2   # (1000, 10)
    y_hat = softmax(z2) # (1000, 10)
    loss = - np.sum(np.multiply(labels, np.log(y_hat)))/data.shape[0]
    return a, y_hat, loss
...
def backward_prop(data, labels, params, forward_prop_func):
    ...
    d1 = y_hat - labels
    g_w2 = a.T @ d1   # (300, 10)
    g_b2 = np.sum(d1, axis=0, keepdims=True)  # (1, 10 )
    d2 = np.multiply(d1 @ W2.T, a * (1-a))
    g_w1 = data.T @ d2  # (784, 300)
    g_b1 = np.sum(d2, axis=0, keepdims=True)  # (1, 300 )
    grad = {'W2': g_w2/B, 'b2': g_b2/B, 'W1': g_w1/B, 'b1': g_b1/B}
    return grad
    # *** END CODE HERE ***
...
def backward_prop_regularized(data, labels, params, forward_prop_func, reg):
    ...
    g_w1 = data.T @ d2   # (784, 300)
    g_b1 = np.sum(d2, axis=0, keepdims=True)  # (1, 300 )
    grad = {'W2': g_w2 / B + 2 * reg * W2, 'b2': g_b2 / B, 'W1': g_w1 / B + 2 * reg * W1, 'b1': g_b1 / B}
    return grad
    # *** END CODE HERE ***
