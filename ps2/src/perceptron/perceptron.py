...
def initial_state():
    ...
    """
    # *** START CODE HERE ***
    state = {'beta': [0], 'x_prev': [np.zeros((2,))]}
    return state
    # *** END CODE HERE ***
...
def predict(state, kernel, x_i):
    ...
    """
    # *** START CODE HERE ***
    x_i_prev = state['x_prev']
    beta = state['beta']
    summation = 0
    for i in range(0, len(x_i_prev)):
        summation += beta[i] * kernel(x_i_prev[i], x_i)
    pred = sign(summation)
    return pred
    # *** END CODE HERE ***
...
def update_state(state, kernel, learning_rate, x_i, y_i):
    ...
    """
    # *** START CODE HERE ***
    state['beta'].append(learning_rate*(y_i - predict(state, kernel, x_i)))
    state['x_prev'].append(x_i)
    # *** END CODE HERE ***
